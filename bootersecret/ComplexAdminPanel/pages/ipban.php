    ...
    if (isset($_POST['addBtn']))
    {
      $ipAdd = $_POST['ipAdd'];
        ...
        if (empty($ipAdd))
          ...
          {
            $SQLinsert = $odb -> prepare("INSERT INTO `ipbanned` VALUES(NULL, :ip)");
    ...
    if (isset($_POST['deleteBtn']))
            {
              $deletes = $_POST['id'];
                ...
                $SQL = $odb -> prepare("DELETE FROM `ipbanned` WHERE `ID` = :id LIMIT 1");
                $SQL -> execute(array(':id' => $deletes));

                $link3 = 'Removed A Banned IP';
$SQLinsert = $odb -> prepare("INSERT INTO `adminlogs` VALUES(NULL, :username, :ip , :page , UNIX_TIMESTAMP())");
...
<th>Delete</th>
          ...
          <tbody>
          <?php
          $SQLSelect = $odb -> query("SELECT * FROM `ipbanned` ORDER BY `ID` DESC");
